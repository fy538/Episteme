...
class BriefSectionSerializer(serializers.ModelSerializer):
    ...
    def get_subsections(self, obj):
        """Return nested subsections (one level deep to avoid recursion)."""
...
class BriefOverviewSerializer(serializers.Serializer):
    ...
    def get_sections(self, brief):
        ...
        for section in sections:
            active_annotations = section.annotations.filter(
                dismissed_at__isnull=True,
                resolved_at__isnull=True
            )
            result.append({
                ...
                'annotation_counts': {
                    'blocking': active_annotations.filter(priority=AnnotationPriority.BLOCKING).count(),
                    'important': active_annotations.filter(priority=AnnotationPriority.IMPORTANT).count(),
                    'info': active_annotations.filter(priority=AnnotationPriority.INFO).count(),
                },
    ...
    def get_overall_grounding(self, brief):
        ...
        sections = BriefSection.objects.filter(brief=brief)
        ...
        status_counts = {}
        for section in sections:
            status = section.grounding_status
