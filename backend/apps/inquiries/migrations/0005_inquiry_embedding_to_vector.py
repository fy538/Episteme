"""
Migrate Inquiry.embedding from JSONField to pgvector VectorField.
...
def convert_json_to_vector(apps, schema_editor):
    ...
    batch_size = 500
    inquiries = Inquiry.objects.exclude(
        embedding_json__isnull=True,
    ).only('id', 'embedding_json')

    to_update = []
    for inquiry in inquiries.iterator(chunk_size=batch_size):
        data = inquiry.embedding_json
        if isinstance(data, list) and len(data) == 384:
            inquiry.embedding_vec = data
            to_update.append(inquiry)

        if len(to_update) >= batch_size:
            Inquiry.objects.bulk_update(to_update, ['embedding_vec'])
            to_update = []
    ...
    operations = [
        ...
        migrations.AddIndex(
            ...
            index=HnswIndex(
                ...
                ef_construction=64,
                m=16,
                name='inquiry_embedding_hnsw_idx',
