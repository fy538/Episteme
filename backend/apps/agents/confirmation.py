...
async def handle_agent_clarification(
    thread: ChatThread,
    clarifying_question: str
) -> str:
    """
    Generate response to user's clarifying question about agent suggestion.
    
    Args:
        thread: Chat thread
        clarifying_question: User's question
    
    Returns:
        Assistant response explaining the agent
    """
    pending = thread.metadata.get('pending_agent_suggestion', {})
    agent_type = pending.get('suggested_agent', 'agent')
    
    provider = get_llm_provider('fast')
    
    prompt = f"""User is asking about a suggested {agent_type} agent.

AGENT TYPE: {agent_type}
WHAT IT WOULD DO: {pending.get('reasoning', '')}
TOPIC: {pending.get('suggested_topic', '')}

USER'S QUESTION: {clarifying_question}

Respond naturally, explaining what the agent would do, how long it might take, and what results they'd get.
Keep it brief (2-3 sentences)."""
    
    response = ""
    async for chunk in provider.stream_chat(
        messages=[{"role": "user", "content": prompt}],
        system_prompt="You explain agent capabilities clearly and concisely."
    ):
