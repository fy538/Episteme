  ...
  const renderBlock = (block: Block, index: number) => {
    ...
    return (
      <div key={block.id} className="mb-4 group">
        {/* Block content */}
        <div className={`relative ${isEditing ? 'hover:bg-neutral-50 rounded p-2' : ''}`}>
          {isCurrentlyEditing ? (
            /* Edit mode */
            <div>
              <Textarea
            ...
            <div className="relative">
              {block.type === 'heading' ? (
                ...
                <p className="text-neutral-800">{block.content}</p>
              )}
              ...
              {isEditing && (
                <button
                  ...
                  className="absolute -right-2 top-0 opacity-0 group-hover:opacity-100 px-2 py-1 bg-white border rounded text-xs text-neutral-600 hover:bg-neutral-50"
                >
                  Edit
                </button>
              )}
              ...
              {block.cites && block.cites.length > 0 && (
                <div className="mt-2 flex gap-1 flex-wrap">
                  {block.cites.map((citeId) => (
                    <span
                      key={citeId}
                      className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-accent-50 text-accent-700 cursor-pointer hover:bg-accent-100"
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-3xl tracking-tight font-bold text-neutral-900">{artifact.title}</h1>
      ...
      {/* Footer */}
      <div className="mt-8 pt-6 border-t text-sm text-neutral-500">
