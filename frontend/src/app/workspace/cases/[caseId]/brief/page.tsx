...
export default function BriefEditorPage() {
  ...
  const handleCreateInquiry = useCallback((selectedText: string) => {
    ...
    router.push(`/workspace/cases/${caseId}/overview?createInquiry=${encodeURIComponent(selectedText)}`);
  }, [caseId, router]);
    ...
    return (
      <div className="flex flex-col min-h-screen bg-neutral-50 dark:bg-primary-950">
        <GlobalHeader
          breadcrumbs={[
            { label: 'Cases', href: '/workspace/cases' },
            { label: caseData?.title || 'Case', href: `/workspace/cases/${caseId}/overview` },
            { label: 'Brief' },
          ]}
          showNav={true}
        />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(`/workspace/cases/${caseId}/overview`)}
            >
