...
> **⚠️ STATUS: ASPIRATIONAL / PARTIALLY IMPLEMENTED**
>
> This document describes a **design vision** for a rich 2D memory system. The actual implementation is significantly simpler — basic signal retrieval exists in `memory_retrieval.py`, but the temperature tiers, access tracking, ARM-style decay, and Celery consolidation tasks described below were **never built**. The companion `memory.py` file has been deleted.
>
> Use this as a reference for future memory system improvements, not as documentation of current behavior. For what actually exists, see the Signal model in `backend/apps/signals/models.py` and `backend/apps/signals/memory_retrieval.py`.
